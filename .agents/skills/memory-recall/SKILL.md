...
## Tag taxonomy
Mandatory on every Codex-mem write and search:
...
Unfiltered search refusal (MEM-02, A8): "A8 — Codex-mem search requires `proj:` and `type:`."
...
## Memory hierarchy
`Codex-mem` owns cross-session intent — decisions, patterns, bugs, gotchas (weeks/months). `context-mode` owns current-session state — in-flight outputs, scratch, ephemeral observations.
...
- same-session resume (ctx-mode has session state for this project): ctx-mode first; fall back to Codex-mem.
- cross-session resume (no ctx-mode session state): Codex-mem first with `proj:` + `type:(decision|pattern|todo)`; skip ctx-mode.
...
- A7: no Codex-mem search for first-time repo onboarding. Refusal: "A7 — no prior memory for an unfamiliar repo; start with gitnexus/filesystem."
- A8: no Codex-mem search without `proj:` and `type:`. Refusal above.
