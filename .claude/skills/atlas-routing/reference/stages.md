...
|---|---|---|---|---|
| **align** | Align | Establishes feature scope, scores risk, and claims work unit. | - Score task risk using `profile-scorer`.<br>- Acquire work unit claim via `claim-manager`.<br>- Record Behavioral Core assumptions, boundaries, and verification target. | - `align-gate` (Judgment: skipped if profile is `FAST`) |
| **trace** | Trace | Performs codebase recon and designs implementation plan. | - Codebase recon via search symbols/references.<br>- Write specification and tasks checklist.<br>- Confirm the smallest sufficient implementation path and scoped write targets. | - `trace-review` (Judgment)<br>- Spec Validation (`validate-spec.js`) |
...
### 1. Align
Before writing any code or plans, the agent must define the scope. The risk scorer analyzes the task parameters to assign a risk profile (`FAST`, `STANDARD`, `FULL`). The `claim-manager` registers task ownership, blocking concurrent execution on the same task. The agent also records Behavioral Core assumptions, write boundaries, and the verification target before moving on.

### 2. Trace
The agent performs reconnaissance on the codebase using symbol analysis, searches, and impact graph tools. A concise implementation spec and task checklist are drafted. The plan should choose the smallest sufficient implementation path and keep write targets inside the accepted scope. Formal validation checks verify that the spec conforms to the required schema.
