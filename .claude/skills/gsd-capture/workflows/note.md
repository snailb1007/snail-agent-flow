...
- **Global scope**: `~/.gemini/antigravity/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `~/.gemini/antigravity/notes/`)
...
2. Glob `~/.gemini/antigravity/notes/*.md` (if directory exists) — global notes
...
Global (~/.gemini/antigravity/notes/):
...
2. **No `.planning/`**: Falls back to global `~/.gemini/antigravity/notes/` — works in any directory
