...
**Secondary:**
- YAML - Spec Kit workflow and extension configuration in `.gemini/.specify/workflows/speckit/workflow.yml`, `.gemini/.specify/extensions.yml`, and `.gemini/.specify/extensions/git/extension.yml`.
...
- Bash - Spec Kit helper scripts in `.gemini/.specify/scripts/bash/check-prerequisites.sh`, `.gemini/.specify/scripts/bash/setup-plan.sh`, `.gemini/.specify/scripts/bash/setup-tasks.sh`, `.gemini/.specify/scripts/bash/create-new-feature.sh`, and the Claude hook `.claude/hooks/check-gstack.sh`.
- JSON - Integration state and Claude settings in `.gemini/.specify/integration.json`, `.gemini/.specify/init-options.json`, `.gemini/.specify/integrations/gemini.manifest.json`, `.gemini/.specify/integrations/speckit.manifest.json`, and `.claude/settings.json`.
...
- Shell execution is expected for Spec Kit scripts under `.gemini/.specify/scripts/bash/`.
...
- Spec Kit `0.8.14.dev0` - Installed/configured for Gemini in `.gemini/.specify/integration.json` and `.gemini/.specify/init-options.json`.
- Gemini Spec Kit integration - Configured as the default integration in `.gemini/.specify/integration.json`; command prompts live in `.gemini/commands/speckit.*.toml`.
...
- Spec Kit workflow commands are described in `.gemini/.specify/workflows/speckit/workflow.yml`.
- Git extension hooks are configured in `.gemini/.specify/extensions.yml` and `.gemini/.specify/extensions/git/extension.yml`.
...
- Spec Kit - Drives `specify -> plan -> tasks -> implement` workflow. Evidence: `.gemini/.specify/workflows/speckit/workflow.yml` requires `speckit_version: ">=0.8.5"`, while `.gemini/.specify/integration.json` records `0.8.14.dev0`.
- Gemini integration - Current Spec Kit integration. Evidence: `.gemini/.specify/integration.json` sets `installed_integrations: ["gemini"]` and `default_integration: "gemini"`.
- Git extension - Adds branch, remote, validation, initialization, and auto-commit hooks for Spec Kit. Evidence: `.gemini/.specify/extensions.yml` and `.gemini/.specify/extensions/git/extension.yml`.
...
- Workflow/configuration files:
  ...
  - `.gemini/.specify/integration.json`: Spec Kit integration state.
  - `.gemini/.specify/init-options.json`: initialized AI/integration options.
  - `.gemini/.specify/extensions.yml`: enabled Git extension hooks.
  - `.gemini/.specify/workflows/speckit/workflow.yml`: full SDD workflow.
  - `.serena/project.yml`: Serena project configuration.

## Platform Requirements

**Development:**
- Git is expected by Spec Kit Git extension commands in `.gemini/.specify/extensions/git/extension.yml`.
- Bash/sh is expected for `.gemini/.specify/scripts/bash/*.sh` and `.claude/hooks/check-gstack.sh`.
- Claude Code users need the global GStack install described in `CLAUDE.md`.
- Gemini Spec Kit commands use `.gemini/commands/speckit.*.toml` and the initialized Gemini integration recorded in `.gemini/.specify/integration.json`.
