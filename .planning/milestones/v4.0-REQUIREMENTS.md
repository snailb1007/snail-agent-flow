...
- [x] **RAOS-01**: Implement a pure task scoring module that rates tasks on a 0-2 scale across 5 dimensions (Novelty, Blast Radius, Ambiguity, Reversibility, User/Biz Risk). Returns FAST (0-2), STANDARD (3-5), or FULL (6+) profile, with override rules for BUGFIX and PROTOTYPE.
...
- [x] **RAOS-04**: Implement Profile-Switch Checkpoints. Generate checkpoint markdown files at `.ai/state/profile-switch-*.md` when switching profiles mid-flight, capturing completed work, assumptions, touched files, risks, and resume steps.
- [x] **RAOS-05**: Enforce durable ADR usage. The validator must block checkins if transient session/profile state is written to `docs/adr/`.

### Observability Signals
- [x] **RAOS-06**: Implement initial 5-signal logger. Record phase duration, revision count, escalation count, test pain notes, and recurring review flags in `.ai/signals/current-period.md`. Enforce that every logged signal must map to a decision it can influence.

### Failure Recovery & CLI Integration
- [x] **RAOS-07**: Implement failure recovery patterns in the engine for blocked spec sign-offs, scope creep, wrong profile choices, and context exhaustion.
- [x] **RAOS-08**: Integrate scoring, claiming, leasing, and signals into the CLI (`adp status`, `adp doctor`, and strict init checks) to assert environment compliance.
...
|-------------|-------|--------|
| RAOS-01 | Phase 18 | Complete |
| RAOS-02 | Phase 19 | Complete |
| RAOS-03 | Phase 20 | Complete |
| RAOS-04 | Phase 21 | Complete |
| RAOS-05 | Phase 21 | Complete |
| RAOS-06 | Phase 22 | Complete |
| RAOS-07 | Phase 23 | Complete |
| RAOS-08 | Phase 23 | Complete |

**Coverage:**
- Active requirements: 8 total, 8 completed, 0 pending.
