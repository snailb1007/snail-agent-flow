...
| Gemini/Spec-Kit adapter | `.gemini/commands/`, `.gemini/.specify/` | Present | Tool-specific command scaffold and shell helpers. |
...
| Spec adapter | Translate Spec-Kit/Gemini commands into artifact operations | `.gemini/commands/`, `.gemini/.specify/` | Canonical policy decisions |
...
3. Spec-Kit/Gemini commands create or update specification artifacts through `.gemini/commands/*.toml` and `.gemini/.specify/scripts/bash/*`.
...
| `.gemini/.specify/` | Spec-Kit adapter | Vendored workflows, templates, scripts, metadata | Keep in sync with path contract but do not make it the sole source of truth. |
...
1. **Canonical artifact contract**
   ...
   - Decide how `.ai/specs/`, `.gemini/.specify/`, and future root `.specify/` paths relate.
...
5. **Adapter alignment**
   - Update `.gemini/commands/`, `.gemini/.specify/`, `.agents/skills/`, and `.claude/hooks/` to consume the same artifact contract.
...
**What goes wrong:** `.ai/specs/`, `.gemini/.specify/`, root `.specify/`, and future CLI paths each become independently authoritative.  
