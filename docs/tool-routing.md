...
| **Gate** | Spec Validation | Deterministic Node.js validator | `npm run validate` / `node validators/scripts/validate-spec.js` | `.specify/feature.json`, `.ai/state/active-feature.json`, `specs/<feature-slug>/{spec,plan,tasks}.md` | Updated `.ai/state/run-state.json`; `.ai/reviews/<feature-slug>/human-review.md` after 3 consecutive failures | Required files, required headings, path drift, placeholder scan, retry circuit breaker | Validator prints `Validation PASSED` or exits blocked; after 3 failures it exits with `NEEDS_HUMAN_REVIEW` |
