...
function runStrictChecks(repoRoot, opts = {}) {
  ...
  const flowPath = '.ai/flows/atlas-flow.yaml';
  ...
  // 3. state.exists & state.schema
  const statePath = '.ai/state/flow-state.json';
  const fullStatePath = path.join(repoRoot, statePath);
  let stateExists = false;
  try {
    stateExists = fs.existsSync(fullStatePath);
  } catch (e) {
    stateExists = false;
  }
  ...
  results.push({
    ...
    subject: statePath,
    evidence: {
      checkedPaths: [statePath],
      parseError: stateExists ? undefined : 'Flow state JSON file is missing.'
    },
    guidance: null
  });
  ...
  if (stateExists) {
    ...
    try {
      const flowState = require('./flow-state');
      const stateJson = fs.readFileSync(fullStatePath, 'utf8');
      const state = JSON.parse(stateJson);
      flowState.validate(state);
      stateValid = true;
    } catch (e) {
      stateParseError = e.message;
    }
    ...
    results.push({
      ...
      category: 'artifact',
      required: true,
      passed: stateValid && !stateParseError,
      subject: statePath,
      evidence: {
        parseError: stateParseError || undefined
      },
      guidance: null
    });
  }
  ...
  for (const p of atlasPaths) {
    ...
    results.push({
      ...
      evidence: {
        checkedPaths: [p],
        parseError: exists ? undefined : `ATLAS runtime asset missing at ${p}`
      },
      guidance: null
    });
  }

  // Non-intrusive smart-default: when a team's instruction file pre-existed, SAF writes its
  // guidance to .ai/instructions/ATLAS.md instead of mutating the file. A required instruction
  // section is therefore satisfied if it appears in the instruction file OR in that separate file.
  let atlasInstructionsContent = '';
  try {
    atlasInstructionsContent = fs.readFileSync(path.join(repoRoot, '.ai/instructions/ATLAS.md'), 'utf8');
  } catch (e) {}
    ...
    if (exists) {
      ...
      try {
        ...
        passed = content.includes('## Subagent & Parallel Execution Guidelines')
          || atlasInstructionsContent.includes('## Subagent & Parallel Execution Guidelines');
      } catch (e) {
        passed = false;
      }
      results.push({
        ...
        subject: f,
        evidence: {
          parseError: passed ? undefined : `Missing heading "## Subagent & Parallel Execution Guidelines" in ${f} or .ai/instructions/ATLAS.md`
        },
        guidance: null
      });
    }
  ...
  // 7b. instructions.subagentGuidelines.current (non-blocking): legacy tool names
  // (define_subagent / invoke_subagent) exist in no supported runtime and mark guidance
  // generated by an older SAF. Warn with refresh steps; never auto-migrate.
  const legacySubagentToolRe = /define_subagent|invoke_subagent/;
  const legacyGuidelineHits = [];
  for (const f of filesToCheck) {
    try {
      const c = fs.readFileSync(path.join(repoRoot, f), 'utf8');
      if (legacySubagentToolRe.test(c)) legacyGuidelineHits.push(f);
    } catch (e) {}
  }
  if (legacySubagentToolRe.test(atlasInstructionsContent)) {
    legacyGuidelineHits.push('.ai/instructions/ATLAS.md');
  }
  results.push({
    id: 'instructions.subagentGuidelines.current',
    category: 'instruction',
    required: false,
    passed: legacyGuidelineHits.length === 0,
    subject: legacyGuidelineHits.join(', ') || 'instruction files',
    evidence: {
      parseError: legacyGuidelineHits.length === 0
        ? undefined
        : `Outdated subagent guidelines (legacy define_subagent/invoke_subagent tool names) in: ${legacyGuidelineHits.join(', ')}. Re-run "adp init" to refresh .ai/instructions/ATLAS.md, then remove the legacy "## Subagent & Parallel Execution Guidelines" section from the listed files (SAF never edits pre-existing instruction files).`
    },
    guidance: null
  });

  // 7c. instructions.memoryCompaction.current (non-blocking): the "Vệt ốc sên" snail-trail
  // section must prescribe a fast model. A section present but missing any prescribed model
  // token is stale guidance from an older SAF — warn with refresh steps, never auto-migrate.
  const compactionHeadingRe = /^##\s+Snail\s+Trail\b.*Memory\s+Compaction/im;
  const prescribedModelRe = /haiku|sonnet|gpt-5\.4-mini|flash|fastest\/cheapest/i;
  const staleCompactionHits = [];
  const filesWithAtlas = [...filesToCheck, '.ai/instructions/ATLAS.md'];
  for (const f of filesWithAtlas) {
    let c = '';
    try {
      c = f === '.ai/instructions/ATLAS.md'
        ? atlasInstructionsContent
        : fs.readFileSync(path.join(repoRoot, f), 'utf8');
    } catch (e) {
      continue;
    }
    if (compactionHeadingRe.test(c) && !prescribedModelRe.test(c)) {
      staleCompactionHits.push(f);
    }
  }
  results.push({
    id: 'instructions.memoryCompaction.current',
    category: 'instruction',
    required: false,
    passed: staleCompactionHits.length === 0,
    subject: staleCompactionHits.join(', ') || 'instruction files',
    evidence: {
      parseError: staleCompactionHits.length === 0
        ? undefined
        : `Stale snail-trail memory-compaction guidelines (no prescribed fast model) in: ${staleCompactionHits.join(', ')}. Re-run "adp init" to refresh .ai/instructions/ATLAS.md, then remove the stale "## Snail Trail — Memory Compaction at Settle" section from any pre-existing instruction files (SAF never edits them).`
    },
    guidance: null
  });
    ...
    if (exists) {
      ...
      try {
        ...
        passed = content.includes('## Context Budget and Subagent Orchestration Policy')
          || atlasInstructionsContent.includes('## Context Budget and Subagent Orchestration Policy');
      } catch (e) {
        passed = false;
      }
      results.push({
        ...
        subject: f,
        evidence: {
          parseError: passed ? undefined : `Missing heading "## Context Budget and Subagent Orchestration Policy" in ${f} or .ai/instructions/ATLAS.md`
        },
        guidance: null
      });
    }
  ...
  // 14. validate-drift checks
  // 14. instructions.behavioralCoreSection
  for (const f of filesToCheck) {
    const fullPath = path.join(repoRoot, f);
    let exists = false;
    try {
    ...
    if (exists) {
      ...
      try {
        content = fs.readFileSync(fullPath, 'utf8');
        passed = content.includes('## Behavioral Core')
          || atlasInstructionsContent.includes('## Behavioral Core');
      } catch (e) {
        passed = false;
      }
      results.push({
        id: 'instructions.behavioralCoreSection',
        category: 'instruction',
        required: true,
        passed,
        subject: f,
        evidence: {
          parseError: passed ? undefined : `Missing heading "## Behavioral Core" in ${f} or .ai/instructions/ATLAS.md`
        },
        guidance: null
      });
    }
  ...
  // 15. validate-drift checks
  try {
    const { validateDrift } = require('./validate-drift');
    const driftResults = validateDrift(repoRoot);
    for (const res of driftResults) {
      if ((res.status === 'FAIL' || res.status === 'BLOCKED') && !opts.skipExit && process.env.NODE_ENV !== 'test') {
        process.stderr.write(`[init-checks] Drift validation FAILED: ${res.check} - ${res.message}\n`);
        process.exit(1);
      }
      results.push({
        id: `drift.${res.check}`,
        category: 'artifact',
        required: res.status === 'FAIL' || res.status === 'BLOCKED',
        passed: res.status === 'PASS',
        subject: res.check,
        evidence: {
          parseError: res.status === 'PASS' ? undefined : res.message
        },
        guidance: null
      });
    }
  ...
  // 16. Memory file existence checks (non-blocking)
  const memoryFiles = [
    'project-summary.md',
  ...
  for (const memFile of memoryFiles) {
    const memPath = path.join(repoRoot, '.ai/memory', memFile);
    const checkId = `memory.${memFile.replace('.md', '').replace(/-/g, '_')}`;
    let memExists = false;
    try {
      memExists = fs.existsSync(memPath);
    } catch (e) {
      memExists = false;
    }
    results.push({
      ...
      required: false,
      passed: memExists,
      subject: `.ai/memory/${memFile}`,
      evidence: {
        checkedPaths: [`.ai/memory/${memFile}`],
        parseError: memExists
          ? undefined
  ...
  // 16b. memory.budgetScope.recommended (non-blocking): an existing context
  // policy without feature-scoping keeps the legacy 'all' behavior, which sums
  // every historical session/pack into the byte-pressure estimate (O(N) bloat).
  // Recommend the switch; never auto-migrate (022 / matrix P1-017).
  {
    let scopeConfigured = true;
    let missingScopeKeys = [];
    let policyPresent = false;
    try {
      const raw = fs.readFileSync(fullPolicyConfigPath, 'utf8');
      policyPresent = true;
      const cfg = JSON.parse(raw);
      const bi = cfg && cfg.budget_inputs;
      missingScopeKeys = [];
      if (!bi || !bi.session_scope) missingScopeKeys.push('budget_inputs.session_scope');
      if (!bi || !bi.context_pack_scope) missingScopeKeys.push('budget_inputs.context_pack_scope');
      scopeConfigured = missingScopeKeys.length === 0;
    } catch (e) {
      // Missing/malformed policy is reported by policy.config.* checks; skip here.
      scopeConfigured = true;
      missingScopeKeys = [];
    }
    const passed = !policyPresent || scopeConfigured;
    results.push({
      id: 'memory.budgetScope.recommended',
      category: 'artifact',
      required: false,
      passed,
      subject: policyConfigPath,
      evidence: {
        ...
        parseError: passed
          ? undefined
          : `context-policy.json is missing ${missingScopeKeys.join(' and ')}; the byte-pressure estimate may count historical sessions or packs. Set budget_inputs.session_scope and context_pack_scope to "active_feature" to scope the estimate to the active feature (the estimate only shrinks; no gate tightens).`
      },
      guidance: null
    });
  }

  // 16c. memory.sessions.archivable (non-blocking): raw session logs that belong
  // to an already-compacted feature (one with a compact-<slug>.json pack) and are
  // not the active feature can be moved out of the active scan surface with
  // `saf compact-memory --archive`. Detect and recommend; never move on doctor.
  {
    let activeSlug = '';
    try {
      const raw = fs.readFileSync(fullFeatJsonPath, 'utf8');
      const data = JSON.parse(raw);
      if (data && data.feature_directory) {
        activeSlug = path.basename(String(data.feature_directory).replace(/[\/\\]+$/, ''));
      }
    } catch (e) {
      // No active pointer: every compacted feature's logs are candidates.
    }

    const compactedSlugs = new Set();
    try {
      for (const f of fs.readdirSync(contextPacksDir)) {
        const m = /^compact-(.+)\.json$/.exec(f);
        if (m && m[1] !== 'adhoc') compactedSlugs.add(m[1]);
      }
    } catch (e) {
      // No packs dir: nothing compacted yet.
    }

    let archivable = 0;
    const archivableSlugs = new Set();
    try {
      const { readSessionFeature } = require('./context-budget');
      const sessionsDir = path.join(repoRoot, '.ai/sessions');
      for (const entry of fs.readdirSync(sessionsDir, { withFileTypes: true })) {
        if (!entry.isFile() || !entry.name.endsWith('.md')) continue;
        const slug = readSessionFeature(path.join(sessionsDir, entry.name));
        if (slug && slug !== activeSlug && compactedSlugs.has(slug)) {
          archivable++;
          archivableSlugs.add(slug);
        }
      }
    } catch (e) {
      // No sessions dir or unreadable: nothing to advise.
    }

    const passed = archivable === 0;
    results.push({
      id: 'memory.sessions.archivable',
      category: 'artifact',
      required: false,
      passed,
      subject: '.ai/sessions',
      evidence: {
        checkedPaths: ['.ai/sessions'],
        parseError: passed
          ? undefined
          : `${archivable} session log(s) belong to already-compacted feature(s) [${[...archivableSlugs].join(', ')}] and still sit in .ai/sessions/. Run "saf compact-memory --archive" to move them to .ai/sessions/archive/<slug>/ (moved, not deleted) and keep the active scan surface bounded.`
      },
      guidance: null
    });
  }

  // 17. skills.version.current (non-blocking): localized SAF skills vs installed package version.
  // Init copies skills no-overwrite, so an upgraded package leaves stale skill copies on
  // disk; the stamp written by `saf init` records which version actually localized them.
  const stampRel = '.ai/state/skills-version.json';
  let stampVersion = null;
  let stampMalformed = false;
  try {
    const stampRaw = fs.readFileSync(path.join(repoRoot, stampRel), 'utf8');
    const stamp = JSON.parse(stampRaw);
    stampVersion = typeof stamp.saf_version === 'string' ? stamp.saf_version : null;
    if (!stampVersion) stampMalformed = true;
  } catch (e) {
    if (e.code !== 'ENOENT') stampMalformed = true;
  }
  ...
  const skillsVersionCurrent = !!packageVersion && stampVersion === packageVersion;
  const refreshHint = 'To refresh: remove the SAF-owned skill folders ' +
    '(.claude/skills/atlas-*, .claude/skills/saf-upgrade, .claude/skills/contracts ' +
    'and their .agents/skills twins), then re-run `saf init`.';
  results.push({
    id: 'skills.version.current',
    category: 'localization',
    required: false,
    passed: skillsVersionCurrent,
    subject: stampRel,
    evidence: {
      checkedPaths: [stampRel],
      parseError: skillsVersionCurrent
        ? undefined
        : (stampVersion
          ? `Localized skills were deployed by SAF ${stampVersion}; installed package is ${packageVersion || 'unknown'}. ${refreshHint}`
          : `Localized skills version unknown (${stampRel} is ${stampMalformed ? 'malformed' : 'missing'}). If skills were localized by an older SAF, they may be stale. ${refreshHint}`)
    },
    guidance: null
  });
...
function formatTerminal(report, source = '') {
  ...
  if (!report.ok) {
    ...
    // Warnings are still surfaced on success — migration.md promises that
    // doctor's warnings list every pending migration.
...
function formatMarkdownGuide(report, meta = {}) {
  ...
  for (const fail of report.failures) {
    if (fail.category === 'instruction') {
      ...
      } else if (fail.id === 'instructions.behavioralCoreSection') {
        lines.push('## Behavioral Core');
        lines.push('- State assumptions before implementation.');
        lines.push('- Prefer the smallest sufficient implementation path.');
        lines.push('- Touch only files and symbols in the accepted scope.');
        lines.push('- Define verification before claiming completion.');
      } else {
