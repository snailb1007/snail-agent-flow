...
class LeaseManager {
  ...
  /**
   * Inspects if a lease is active for a file without mutating state.
   *
   * @param {string} file - Path to the target file.
   * @returns {object} Object with details on lease status.
   */
  inspect(file) {
    const key = this.getFileKey(file);
    const lockFilePath = path.join(this.store.dirPath, key + '.json');
    ...
    try {
      ...
      return {
        ...
        target_file: metadata.target_file || null
      };
    } catch (e) {
      return { held: false };
    }
