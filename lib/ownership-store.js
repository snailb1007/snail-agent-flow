...
/**
 * OwnershipStore — LOCAL, SINGLE-MACHINE advisory locks (claims & leases).
 *
 * Scope and limitations (read before relying on this for coordination):
 *  - Liveness is detected with `process.kill(pid, 0)`, which only resolves processes on the
 *    SAME machine. A lock acquired by a developer on another machine is meaningless here — its
 *    PID won't exist locally, so it is treated as stale and may be stolen.
 *  - These are therefore advisory, single-session locks for coordinating parallel subagents on
 *    one machine. They do NOT provide cross-developer, cross-machine, or distributed coordination.
 *  - Do not commit `.ai/locks` / `.ai/claims` to share locks across a team — it creates merge
 *    noise and is still not atomic. Use git branches/PRs for cross-developer coordination.
 *  See docs/flow/artifact-ownership.md for the full ownership policy.
 */
